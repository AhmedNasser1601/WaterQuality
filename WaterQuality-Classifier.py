...
Waterfile=pd.read_csv("water_potability.csv")
X = Waterfile.iloc[0:3275,: ]     #Features
Y = Waterfile['Potability']       #Label
Waterfile=Waterfile.dropna()      #removing nulls
X = ((X - X.mean()) / X.std())    #Normalization

# Split dataset into training set and test set
X_train, X_test, y_train, y_test = train_test_split(X , Y , test_size = 0.20 , random_state = 1 , shuffle = True) # 80% training and 20% test
